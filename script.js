...
var generatePassword = function() {
  ...
  displayPassword = generatePasswordRandomness(userInput,length);

  function generatePasswordRandomness(userInput, length) {
  ...
  function passLenght () { // This functions validate the user input for lenght of characters from 8-128
    ...
    var length = parseInt(window.prompt("Please enter length")); // Convert to ingteger to check valid input.
    if (length < 8 || length > 128) {
      
      window.alert("Please enter a valid number between 8-128");
      passLenght();

    } else if (!length) { // If null or Nan we will send the user back to the passLenght() function.
      window.alert("Please enter a valid number between 8-128")
      passLenght(); // Recurcion back to our function the get lenght.
    } else {
      window.alert("You have entered a valid length of: " + length + "\n Password will now be generated");
      return length; // We have a valid lengh, lets return our length value.
    }
  }
}
